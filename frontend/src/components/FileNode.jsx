...
const FileNode = ({
  ...
  const icon =
    node.type === "folder" ? (
      <Folder
        size={14}
        className={`mr-2 ${isOpen ? "text-gray-400" : "text-gray-500"}`}
      />
    ) : (
    ...
    <div className="text-sm font-mono dark:text-gray-300 text-gray-500">
      {isRenaming === node.id ? (
        ...
        >
          ...
          />
          <button onClick={saveRename} className="ml-2 text-gray-400">
            <Check size={16} />
          </button>
        </div>
      ) : (
        <div
          ...
          style={{ paddingLeft: `${depth * 10}px` }}
          onClick={() =>
            node.type === "folder" ? setIsOpen(!isOpen) : onSelect(node)
          }
        >
          ...
          <div className="flex space-x-1 opacity-100 transition">
            <button onClick={startRename} className="text-gray-400">
              <Edit size={12} />
            </button>
            <button
              onClick={(e) => {
            ...
            {node.type === "folder" && (
              <>
                <button
                  onClick={(e) => {
                    e.stopPropagation();
                    onAction("add", node.id, { nodeType: "file" });
                  }}
                  className="text-gray-400"
                >
                  <Plus size={12} />
                ...
                <button
                  onClick={(e) => {
                    e.stopPropagation();
                    onAction("add", node.id, { nodeType: "folder" });
                  }}
                  className="text-gray-400"
                >
                  <Folder size={12} />
