...
function FileNode({ node, depth, onSelect, selectedId, onAddFile, onAddFolder, onRename, onDelete, isRenaming, setIsRenaming }) {
...
export default function FileExplorerPanel({ fileTree, onSelect, onAddFile, onAddFolder, onRename, onDelete, selectedId, isRenaming, setIsRenaming }) {
  return (
    <div className="overflow-y-auto h-full dark:bg-black bg-gray-300 dark:text-white text-gray-900">
      ...
      <div className="px-2">
        {fileTree.map((node) => (
          <FileNode key={node.id} node={node} depth={0} onSelect={onSelect} selectedId={selectedId} onAddFile={onAddFile} onAddFolder={onAddFolder} onRename={onRename} onDelete={onDelete} isRenaming={isRenaming} setIsRenaming={setIsRenaming} />
        ))}
      </div>
    </div>
  );
